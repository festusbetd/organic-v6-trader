...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row" style="padding-bottom: 20px">
            ...
            <div class="card">
                <div class="card-body" style="padding: 20px">
                    <h5 class="text-center">{{\App\CPU\translate('mpesa_pay')}}</h5>
                    @php($config=\App\CPU\Helpers::get_business_settings('mpesa_pay'))
                    <form action="{{env('APP_MODE')!='demo'?route('admin.business-settings.payment-method.update',['mpesa_pay']):'javascript:'}}"
                          method="post">
                        @csrf
                        @if(isset($config))
                            <div class="form-group mb-2">
                                <label class="control-label">{{\App\CPU\translate('mpesa_pay')}}</label>
                            </div>
                            <div class="form-group mb-2 mt-2">
                                <input type="radio" name="status" value="1" {{$config['status']==1?'checked':''}}>
                                <label style="padding-left: 10px">{{\App\CPU\translate('Active')}}</label>
                            ...
                            <div class="form-group mb-2">
                                ...
                                <span class="btn btn-secondary btn-sm m-2" onclick="copyToClipboard('#id_mpesa_pay')"><i class="tio-copy"></i> Copy URI</span>
                                <br>
                                <p class="form-control" id="id_mpesa_pay">{{ url('/') }}/mpesa-pay</p>
                            </div>

                            <div class="form-group mb-2">
                                <label
                                    style="padding-left: 10px">Mpesa Pass Key</label><br>
                                <input type="text" class="form-control" name="mpesa_pass_key"
                                       value="{{env('APP_MODE')!='demo'?$config['mpesa_pass_key']:''}}">
                            </div>

                            <div class="form-group mb-2">
                                <label
                                    style="padding-left: 10px">MPESA BUSINESS SHORTCODE</label><br>
                                <input type="text" class="form-control" name="mpesa_business_short_code"
                                       value="{{env('APP_MODE')!='demo'?$config['mpesa_business_short_code']:''}}">
                            </div>
                            <div class="form-group mb-2">
                                <label
                                    style="padding-left: 10px">MPESA CONSUMER KEY</label><br>
                                <input type="text" class="form-control" name="mpesa_consumer_key"
                                       value="{{env('APP_MODE')!='demo'?$config['mpesa_consumer_key']:''}}">
                            </div>
                            <div class="form-group mb-2">
                                <label
                                    style="padding-left: 10px">MPESA CONSUMER SECRET</label><br>
                                <input type="text" class="form-control" name="mpesa_consumer_secret"
                                       value="{{env('APP_MODE')!='demo'?$config['mpesa_consumer_secret']:''}}">
                            </div>
                            <button type="{{env('APP_MODE')!='demo'?'submit':'button'}}" onclick="{{env('APP_MODE')!='demo'?'':'call_demo()'}}" class="btn btn-primary mb-2">{{\App\CPU\translate('save')}}</button>
                        @else
                            <button type="submit"
        ...
        <div class="col-md-6">
                <div class="card">
                    <div class="card-body" style="padding: 20px">
                        <h5 class="text-center">{{\App\CPU\translate('credit_pay')}}</h5>
                        @php($config=\App\CPU\Helpers::get_business_settings('credit_pay'))
                        <form action="{{route('admin.business-settings.payment-method.update',['credit_pay'])}}"
                              style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                              method="post">
                            @csrf
                            @if(isset($config))
