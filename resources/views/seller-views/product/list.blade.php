                        ...
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tr>
                                    ...
                                    <th>{{\App\CPU\translate('Cost')}} {{\App\CPU\translate('Per')}} {{\App\CPU\translate('Item')}}</th>
                                ...
                                @foreach($products as $k=>$p)
                                    <tr>
                                        ...
                                        <td>
                                        <label class="switch">
                                            <input type="checkbox" class="unit_pricing"
                                                   id="{{$p['id']}}" {{$p->unit_pricing == 1?'checked':''}}>
...
@push('script')
    ...
    <script>
        ...
        $(document).on('change', '.unit_pricing', function () {
          
          var id = $(this).attr("id");
          if ($(this).prop("checked") == true) {
              var unit_pricing = 1;
              toastr.success('Unit Pricing Update updated successfully');
          } else if ($(this).prop("checked") == false) {
              var unit_pricing = 0;
          }
          $.ajaxSetup({
          ...
          $.ajax({
              url: "{{route('seller.product.unit_pricing_update')}}",
              method: 'POST',
              data: {
                  id: id,
