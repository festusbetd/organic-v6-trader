                ...
                @foreach($group as $cart_key=>$cartItem)
                    ...
                    <!-- choosen shipping method-->
                        ...
                        @if($shippingMethod=='sellerwise_shipping')
                            ...
                            <div class="row">
                                <div class="col-12">
                                <label for="exampleInputEmail1">Suggested Collection Centre<span style="color: red">*</span></label>
                                    <select class="form-control"
                                            onchange="set_shipping_id(this.value,'{{$cartItem['cart_group_id']}}')">
                                        <option>{{\App\CPU\translate('select_collection_center')}}</option>
                                ...
                                <div class="col-12">
                                <label for="exampleInputEmail1">Collection Centre Opening Hours<span style="color: red">*</span></label>
                                    <select class="form-control"
                                            onchange="set_shipping_id(this.value,'{{$cartItem['cart_group_id']}}')" disabled>
                              
                                        @foreach($shippings as $shipping)
                                            <option
                                                value="{{$shipping['id']}}" {{$choosen_shipping['shipping_method_id']==$shipping['id']?'selected':''}}>
                                                {{$shipping['shop_openings'] }}   ( {{$shipping['shop_opening_time']}} - {{$shipping['shop_closing_time']}} )  
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                <label for="exampleInputEmail1">Collection Centre Closed Days<span style="color: red">*</span></label>
                                    <select class="form-control"
                                            onchange="set_shipping_id(this.value,'{{$cartItem['cart_group_id']}}')" disabled>
                                        @foreach($shippings as $shipping)
                                            <option
                                                value="{{$shipping['id']}}" {{$choosen_shipping['shipping_method_id']==$shipping['id']?'selected':''}}>
                                                {{$shipping['shop_closed_days']}}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                <label for="exampleInputEmail1">Suggested Collection Date<span style="color: red">*</span></label>
                                <input type="date" class="form-control"  name="order_collection_date" id="order_collection_date"  > {{ session('order_collection_date')}}</input>
                              
                                <div class="col-12">
                                <label for="exampleInputEmail1">Suggested Collection Time<span
                                        style="color: red">*</span></label>
                                <input type="time" class="form-control" name="order_collection_time" id="order_collection_time" min="08:00" min="19:00" pattern="(09|1[0-5]):[0-5]\d">{{ session('order_collection_time')}} </input>
                                </div>
                            </div>
                        @endif
                    @endif
...
document.getElementsByName("order_collection_date")[0].setAttribute('min', today);
...
<script>
    function checkout(){
        ...
        let order_collection_date =  $('#order_collection_date').val();
        let order_collection_time= $('#order_collection_time').val();
         
         console.log(order_collection_date);

        $.post({
            url: "{{route('order_collection_date')}}",
            data: {
                    _token: '{{csrf_token()}}',
                    order_collection_date:order_collection_date,
        ...
        $.post({
            url: "{{route('order_collection_time')}}",
            data: {
                    _token: '{{csrf_token()}}',
                    order_collection_time:order_collection_time,
