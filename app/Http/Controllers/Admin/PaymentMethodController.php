...
class PaymentMethodController extends Controller
{
    ...
    public function update(Request $request, $name)
    {
        ...
        if ($name == 'mpesa_before_delivery') {
            $payment = BusinessSetting::where('type', 'mpesa_before_delivery')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'type' => 'mpesa_before_delivery',
                    'value' => json_encode([
                        'status' => $request['status']
                    ]),
                    'created_at' => now(),
                ...
                DB::table('business_settings')->where(['type' => 'mpesa_before_delivery'])->update([
                    'type' => 'mpesa_before_delivery',
                    'value' => json_encode([
                        'status' => $request['status']
                    ]),
        ...
        elseif ($name == 'mpesa_pay') {
            $payment = BusinessSetting::where('type', 'mpesa_pay')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'type' => 'mpesa_pay',
                    'value' => json_encode([
                ...
                if ($request['status'] == 1) {
                    $request->validate([
                        'mpesa_pass_key' => 'required',
                        'mpesa_business_short_code' => 'required',
                        'mpesa_consumer_key' => 'required',
                        'mpesa_consumer_secret' => 'required',
                        // 'mpesa_stk_url' => 'required',
                        // 'mpesa_callback_url' => 'required',
                        // 'mpesa_access_token_url' => 'required',
                    ]);
                }

                DB::table('business_settings')->where(['type' => 'mpesa_pay'])->update([
                    'type' => 'mpesa_pay',
                    'value' => json_encode([
                        'status' => $request['status'],
                        'mpesa_pass_key' =>  $request['mpesa_pass_key'],
                        'mpesa_business_short_code' =>  $request['mpesa_business_short_code'],
                        'mpesa_consumer_key' => $request['mpesa_consumer_key'],
                        'mpesa_consumer_secret' =>  $request['mpesa_consumer_secret'],
                        // 'mpesa_stk_url' =>  $request['mpesa_stk_url'],
                        // 'mpesa_callback_url' =>  $request['mpesa_callback_url'],
                        // 'mpesa_access_token_url' => $request['mpesa_access_token_url'],
                    ]),
                    'updated_at' => now(),
                ]);
            }
