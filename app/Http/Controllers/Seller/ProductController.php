...
class ProductController extends Controller
{
    ...
    public function add_new_user()
    {
        $cat = Category::where(['parent_id' => 0])->get();
        $br = Brand::orderBY('name', 'ASC')->get();
        return view('seller-views.product.add-new-user', compact('cat', 'br'));
    }
    ...
    public function unit_pricing_update(Request $request)
    {
        $product = Product::where(['id' => $request['id']])->first();
        $product->unit_pricing = $request['unit_pricing'];
        $product->save();
        return response()->json([
        'success' => $request['unit_pricing'],
        ], 200);
    }
                ...
                ->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->orWhere('f_name', 'like', "%{$value}%")
                            ...
                            ->orWhere('email', 'like', "%{$value}%")->where(['id' => \auth('seller')->id()]);
                    }
                });
            $query_param = ['search' => $request['search']];
        } else {
            //$sellers = Seller::with(['orders', 'product']);
            $sellers = Seller::where(['id' => \auth('seller')->id()]);
        }
        $sellers = $sellers->latest()->paginate(Helpers::pagination_limit())->appends($query_param);

        // return $sellers;

        return view('seller-views.product.users', compact('sellers', 'search'));
    }
    public function createUser(Request $request)
    {
        ...
        return redirect()->route('seller.user.users');
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'brand_id' => 'required',
            'unit' => 'required',
            // 'unit_pricing' => 'required',
            'tax' => 'required|min:0',
            'purchase_price' => 'required|numeric|min:1',
        ], [
            'name.required' => 'Product name is required!',
            'unit_pricing.required' => 'Unit pricing is required!',
            'category_id.required' => 'category  is required!',
            'brand_id.required' => 'brand  is required!',
