...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        $order_collection_date = session()->has('order_collection_date') ? session('order_collection_date') : null;
        $order_collection_time = session()->has('order_collection_time') ? session('order_collection_time') : null;
        ...
        $or = [
            ...
            'customer_id_number' => $user->user_id,
            ...
            'unit_pricing' => $data['unit_pricing'],
        ...
        foreach (CartManager::get_cart($data['cart_group_id']) as $c) {
            ...
            $qty = $c['quantity'];
            $price = $c['price'];
            if ($data['unit_pricing'] == 1) {
                $total_cost =($c['price'] * $c['quantity'])+(CartManager::get_shipping_cost($data['cart_group_id'])*$c['quantity']);
            } else {
                $total_cost = ($c['price'] * $c['quantity'])+CartManager::get_shipping_cost($data['cart_group_id']);
            }
            $or_d = [
                'order_id' => $order_id,
        ...
        try {
            ...
            // Mail::to($user->email)->send(new \App\Mail\OrderPlaced($order_id,$order_amount));
      ...
      // Mail::to($user->email)->send(new \App\Mail\OrderPlaced($order_id,$order_amount));
            //Mail::to($user->email)->send(new \App\Mail\OrderPlaced($order_id,$value));
