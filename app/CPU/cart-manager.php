...
class CartManager
{
    ...
    public static function cart_clean($request = null)
    {
        ...
        session()->forget('order_collection_date');
        session()->forget('order_collection_time');
    }
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        $cart['unit_pricing'] =$product->unit_pricing;
        ...
        if ($user == 'offline') {
            ...
            $cart_same_seller = Cart::where(['customer_id' =>  $cart['customer_id']])->first();
            if($cart_same_seller==null){
                $cart->save();
            }
            else {
                $cart_same_seller_1 = Cart::where(['seller_id' => $cart['seller_id']])->first();
                if($cart_same_seller_1==null){
                    return 'Customer and seller_id';
                }else{
                    $cart->save();
